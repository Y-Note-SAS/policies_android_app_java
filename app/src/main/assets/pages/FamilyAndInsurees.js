$(document).ready(function () {
    ...
    $('#btnNewInsuree').click(function () {
        ...
        Android.launchActivity("Insuree", args);
    });


    $('.ulList li').click(function () {
        InsureeId = parseInt($(this).find('#hfInsureeId').val());
        console.log(InsureeId);
        isHead = parseInt($(this).find('#hfIsHead').val()) || 0;
        console.log(isHead);
        ExceedThreshold = parseInt($(this).find('#hfExceedThreshold').val()) || 0;
        console.log(ExceedThreshold);
        PolicyId = parseInt($(this).find('#hfPolicyId').val()) || 0;
        ...
        // Android.launchActivity("Insuree", argsString);
    });
    ...
    contextMenu.createContextMenu([Android.getString('Edit'), Android.getString('Delete')], function () {
        ...
        if (clicked == Android.getString('Edit')) {
            ...
            //Android.SetUrl(url);
            //window.open("Insuree.html?i=" + InsureeId + "&f=" + FamilyId, "_self");
            // (InsureeId, FamilyId, isHead, ExceedThreshold, PolicyId);
            let args = {
                "InsureeId": InsureeId,
                "FamilyId": FamilyId,
                "isHead": isHead,
